...
class Player:
    ...
    def generate_start_cards(self):
        ...
        i = 0
        while i < 2:
            my_cards.append(self.get_card())  # # to add random card in list my_cards
            i += 1
        return my_cards
    ...
    def plus_one_card(self, answer):
        ...
        if answer == 'Y':
            self.my_cards.append(self.get_card())  # to add one random card in value self.my_cards

    def values_result(self):
        """Func to append natural value of card, because cards is objects of class Card. """
        list_values = []  # if player has aces ('A'), this list will contain int numbers and tuples (1, 11)
    ...
    def get_rate(self, rate, result):
        """Func to put a rate in game. """
        if int(rate) > self.count:
            return "Sorry, you don't have so much money"
        else:
            if result[8:] == 'Dealer':
                # if winner is dealer, player's count will decrease
                self.count -= int(rate)
